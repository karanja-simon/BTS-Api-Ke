...
const BtsEconet_1 = require("../models/BtsEconet");
class BtsEconetController {
    constructor() {
    }
    getAllBts(req, res) {
        console.log('Get Econet BTS invoked');
        BtsEconet_1.default.find({})
            .then((data) => {
            if (data.length < 1) {
                res.json({
                    status: 204,
    ...
    getBtsByLacRange(req, res) {
        if (Validator_1.default.isValidateRange(req)) {
            BtsEconet_1.default.find({ lac: { $lte: req.body.to, $gte: req.body.from } })
                .then((data) => {
                if (data.length < 1) {
                    res.json({
                        status: 204,
    ...
    getBtsLatLon(req, res) {
        if (Validator_1.default.isValidateRequest(req)) {
            BtsEconet_1.default.find({ mcc: req.body.mcc, mnc: req.body.mnc, lac: req.body.lac, cellid: req.body.cellid })
                .then((data) => {
                if (data.length < 1)
                    res.json({
                        status: 204,
...
exports.default = new BtsEconetController();
