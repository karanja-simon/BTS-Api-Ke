...
const BtsAirtel_1 = require("../models/BtsAirtel");
class BtsAirtelController {
    constructor() {
    }
    getAllBts(req, res) {
        BtsAirtel_1.default.find({})
            .then((data) => {
            if (data.length < 1) {
                res.json({
                    status: 204,
    ...
    getBtsByLacRange(req, res) {
        if (Validator_1.default.isValidateRange(req)) {
            BtsAirtel_1.default.find({ lac: { $lte: req.body.to, $gte: req.body.from } })
                .then((data) => {
                if (data.length < 1) {
                    res.json({
                        status: 204,
    ...
    getBtsLatLon(req, res) {
        if (Validator_1.default.isValidateRequest(req)) {
            BtsAirtel_1.default.find({ mcc: req.body.mcc, mnc: req.body.mnc, lac: req.body.lac, cellid: req.body.cellid })
                .then((data) => {
                if (data.length < 1)
                    res.json({
                        status: 204,
...
exports.default = new BtsAirtelController();
