...
import TelcomBts from '../models/BtsTelcom';
...
class BtsTelcomController {
    ...
    public getBtsByLacRange(req: Request, res: Response): void {
        if (Validator.isValidateRange(req)) {
            TelcomBts.find({lac: {$lte: req.body.to, $gte: req.body.from}})
                .then((data) => {
                    if (data.length < 1) {
                        res.json({
                            status: 204,
    ...
    public getBtsLatLon(req: Request, res: Response): void {
        if (Validator.isValidateRequest(req)) {
            TelcomBts.find({mcc: req.body.mcc, mnc: req.body.mnc, lac: req.body.lac, cellid: req.body.cellid})
                .then((data) => {
...
export default new BtsTelcomController();